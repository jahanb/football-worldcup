...
async function updateQuarterfinalFinal() {
  try {
    ...
    // Get all Quarter-final matches from DB
    const quarterFinals = await Match.find({
      group: 'Quarter-final'
    }).sort({ startTime: 1 });

    console.log(`Found ${quarterFinals.length} Quarter-final matches in database\n`);
    console.log('🏆 QUARTER-FINAL MATCHES (ALL 4):\n');
    console.log('━'.repeat(70) + '\n');

    const updates = [];
    let unchanged = 0;

    for (const dbMatch of quarterFinals) {
    ...
    console.log('━'.repeat(70) + '\n');
    console.log('📊 SUMMARY\n');
    console.log(`Total Quarter-final matches: ${quarterFinals.length}`);
    ...
    if (isTestMode) {
      ...
      console.log('  node update-quarterfinal-final.js\n');
      process.exit(0);
    }
    ...
    if (failCount === 0 && successCount > 0) {
      console.log('🎉 ALL 4 QUARTER-FINAL MATCHES UPDATED!\n');
      console.log('Your Quarter-final matchups:');
...
connectDatabase().then(() => updateQuarterfinalFinal());
