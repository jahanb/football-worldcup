...
async function main() {
  ...
  // Get group parameter from command line
  const groupArg = process.argv[2];
  let targetGroup = null;
  
  if (groupArg) {
    targetGroup = groupArg.toUpperCase();
    console.log(`🎯 Target Group: ${targetGroup}\n`);
  } else {
    console.log('📋 Processing ALL groups (no group specified)\n');
  }
  ...
  const filteredMatches = targetGroup 
    ? matches.filter(match => match.group === targetGroup)
    : matches;
  
  if (filteredMatches.length === 0) {
    console.log(`❌ No matches found for group ${targetGroup}`);
    await client.close();
    return;
  }
  
  console.log(`📊 Found ${filteredMatches.length} matches to predict\n`);
  ...
  for (let i = 0; i < filteredMatches.length; i++) {
    ...
    console.log(`[${i + 1}/${filteredMatches.length}] ${homeTeam} vs ${awayTeam} (Group ${match.group})`);
  ...
  console.log(`📊 Total matches: ${filteredMatches.length}`);
  console.log('='.repeat(60));
